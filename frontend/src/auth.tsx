...
type AuthValue = { user: User | null; loading: boolean; login: (u: string, p: string) => Promise<void>; logout: () => void; refreshUser: () => Promise<void> };
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { if (tokenStore.get()) refreshUser().catch(() => tokenStore.clear()).finally(() => setLoading(false)); else setLoading(false); }, []);
